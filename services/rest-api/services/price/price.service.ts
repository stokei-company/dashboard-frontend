...
export class PriceServiceRest extends BaseService {
    ...
    async addDiscount(priceId: string, data: AddDiscountDTO): Promise<PriceModel> {
        try {
            const response = await this.client.post<PriceModel>(`/prices/${priceId}/discount`, data);
            if (response?.data) {
    ...
    async deleteDiscount(priceId: string): Promise<boolean> {
        try {
            const response = await this.client.delete<{ ok: boolean }>(`/prices/${priceId}/discount`);
            if (response?.data?.ok) {
                return true;
            }
        } catch (error) { }
