...
export class SkuServiceRest extends BaseService {
    ...
    async addQuantity(skuId: string, data: AddQuantitySkuDTO): Promise<boolean> {
        try {
            const response = await this.client.patch<{ ok: boolean }>(`/skus/${skuId}/quantity/add`, data);
            if (response?.data?.ok) {
                return true;
            }
        } catch (error) { }
    ...
    async withdrawQuantity(skuId: string, data: WithdrawQuantitySkuDTO): Promise<boolean> {
        try {
            const response = await this.client.patch<{ ok: boolean }>(`/skus/${skuId}/quantity/withdraw`, data);
            if (response?.data?.ok) {
                return true;
            }
        } catch (error) { }
    ...
    async findAllPrices(skuId): Promise<PriceModel[]> {
        try {
            const response = await this.client.get<PriceModel[]>(`/skus/${skuId}/prices`);
            if (response?.data && response?.data.length > 0) {
                return response.data;
            }
        } catch (error) { }
