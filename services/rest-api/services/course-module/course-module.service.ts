...
export class CourseModuleServiceRest extends BaseService {
    ...
    async create(data: CreateCourseModuleDTO): Promise<ModuleModel> {
        try {
            const response = await this.client.post<ModuleModel>(`/courses/${this.courseId}/modules`, data);
            if (response?.data) {
    ...
    async delete(moduleId: string): Promise<ModuleModel> {
        try {
            const response = await this.client.delete<ModuleModel>(`/modules/${moduleId}`);
            if (response?.data) {
                return response.data;
            }
        } catch (error) { }
    ...
    async findAll(): Promise<ModuleModel[]> {
        try {
            const response = await this.client.get<ModuleModel[]>(`/courses/${this.courseId}/modules`);
