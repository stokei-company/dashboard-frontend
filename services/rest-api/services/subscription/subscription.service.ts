...
export class SubscriptionServiceRest extends BaseService {
    ...
    async start(id: string): Promise<boolean> {
        try {
            const response = await this.client.patch<{ ok: boolean }>(`/subscriptions/${id}/start`);
            if (response?.data?.ok) {
                return true;
            }
        } catch (error) { }
    ...
    async findAll(data?: { status?: string, createdAt?: string, page?: string, limit?: string }): Promise<FindAllPayload<SubscriptionModel>> {
        try {
            const requestData = Object.entries(data || {});
            const params = requestData.length > 0 ? requestData.map(([key, value]) => `${key}=${value}`).join("&") : "";
            const response = await this.client.get<FindAllPayload<SubscriptionModel>>(`/subscriptions${requestData?.length ? "?" + params : ""}`);
            if (response?.data) {
                return response.data;
            }
        } catch (error) { }
