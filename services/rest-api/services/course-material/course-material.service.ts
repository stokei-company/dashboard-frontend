...
export class CourseMaterialServiceRest extends BaseService {
    ...
    async create(data: CreateCourseMaterialDTO | FormData): Promise<MaterialModel> {
        try {
            const response = await this.client.post<MaterialModel>(`/courses/${this.courseId}/materials`, data, {
                headers: {
    ...
    async delete(materialId: string): Promise<MaterialModel> {
        try {
            const response = await this.client.delete<MaterialModel>(`/courses/${this.courseId}/materials/${materialId}`);
            if (response?.data) {
                return response.data;
            }
        } catch (error) { }
    ...
    async findAll(): Promise<MaterialModel[]> {
        try {
            const response = await this.client.get<MaterialModel[]>(`/courses/${this.courseId}/materials`);
