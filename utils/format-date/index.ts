...
export const differenceDate = (dateStart: string, dateEnd: string) => {
    ...
    let response: string[] = [];
    if (interval.years > 0) {
        response = [...response, `${interval.months} ${interval.years === 1 ? "ano" : "anos"}`];
    }
    if (interval.months > 0) {
        response = [...response, `${interval.months} ${interval.months === 1 ? "mes" : "meses"}`];
    }
    if (interval.days > 0) {
        response = [...response, `${interval.days} ${interval.days === 1 ? "dia" : "dias"}`];
    }
    if (interval.hours > 0) {
        response = [...response, `${(interval.hours).toFixed(0)} ${interval.hours === 1 ? "hora" : "horas"}`];
    }
